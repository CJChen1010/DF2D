...
def dc_fov(name, indir, regex, rounds, chans, codebook, min_norm, ENargs, elbow_thrs, chanCoefs, min_peak, outdir, gaus_sigma, weight_thresh):
    print(name + " started")
...
dcpartial = partial(dc_fov, indir=deepcopy(in_dir), regex=deepcopy(regex_3d), rounds=deepcopy(rnds), 
                    ...
                    elbow_thrs=elbow_thresholds, gaus_sigma=weight_sigma, weight_thresh=min_weight)

t1 = time.time()
fov_names = fov_names[214:]
# for name in fov_names:
#     print(name)
#     dcpartial(name)
Parallel(n_jobs=dc_npool, prefer='processes', timeout=100000)(delayed(dcpartial)(name) for name in fov_names)
t2 = time.time()
